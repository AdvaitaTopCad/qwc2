    ...
    switch (action.type) {
        ...
        case SEARCH_CHANGE:
            UrlParams.updateParams({
                ...
                sp: action.providers ? action.providers.join(',') : undefined,
            });
        ...
        case SEARCH_ADD_RESULTS:
            if (
                state.requestId !== action.results.reqId ||
                !(state.pendingProviders || []).includes(
            ...
            pendingProviders = state.pendingProviders.slice(0);
            pendingProviders.splice(
                pendingProviders.indexOf(action.results.provider),
                1
            );
            return assign({}, state, {
                results,
