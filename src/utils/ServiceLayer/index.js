...
export const getDCPTypes = (dcpTypes) => {
    let result = {};
...
export const getWMSLayers = (capabilitiesXml, asGroup = false) => {
    ...
    try {
        ...
        serviceUrl = getDCPTypes(capabilities.Capability.Request.GetMap.DCPType)
...
export const getWFS10Layers = (capabilities) => {
    let serviceUrl = null;
    const { version } = capabilities;
    let formats = null;
    try {
        serviceUrl = getDCPTypes(
    ...
    const layers = [];
    array(capabilities.FeatureTypeList.FeatureType).forEach((featureType) => {
        let name;
        let bbox;
        try {
...
export const getWFS11_20Layers = (capabilities) => {
    let serviceUrl = null;
    const { version } = capabilities;
    let formats = null;
    try {
        ...
        serviceUrl = getDCPTypes(array(getFeatureOp.DCP)).HTTP.Get.href;
...
export const getWFSLayers = (capabilitiesXml) => {
    const options = {
        attrPrefix: '',
    ...
    if (capabilities.WFS_Capabilities.version < '1.1.0') {
        return getWFS10Layers(capabilities.WFS_Capabilities);
    }
    return getWFS11_20Layers(capabilities.WFS_Capabilities);
};

export const findLayers = (type, serviceUrl, layerConfigs, callback) => {
    ...
    axios
        ...
        .then((response) => {
            layerConfigs.forEach((layerConfig) => {
                let result = null;
                if (type === 'wms') {
                    result = getWMSLayers(response.data);
                } else if (type === 'wfs') {
                    result = getWFSLayers(response.data);
                }
