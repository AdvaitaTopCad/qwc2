...
export function getGoogleMercatorScales(
    ...
    const retval = [];
    for (let l = minZoom; l <= maxZoom; l += 1) {
        retval.push(
            (2 * Math.PI * RADIUS) /
                ((TILE_WIDTH * ZOOM_FACTOR ** l) / dpi2dpm(dpi))
        );
    }
    return retval;
}
...
export function getZoomForExtent(
    ...
    if (getConfigProp('allowFractionalZoom') === true) {
        return Math.max(
            minZoom,
            Math.min(this.computeZoom(resolutions, extentResolution), maxZoom)
        );
    }
    const { zoom } = resolutions.reduce(
        (previous, resolution, index) => {
            const diff = Math.abs(resolution - extentResolution);
            return diff > previous.diff ? previous : { diff, zoom: index };
        },
